...
int builtin_cmd(char **argv)
{
    ...
    char *ag = argv[0];
    //若命令为quit
    if(strcmp(ag,"quit")==0)exit(0);
    //若命令为jobs
    else if(strcmp(ag,"jobs")==0)
    {
        //阻塞所有信号，使得该过程中jobs不变
    ...
    else if(strcmp(ag,"bg")==0||strcmp(ag,"fg")==0)
    {//若命令为bg或fg
        do_bgfg(argv);
        return 1;
    }
...
void waitfg(pid_t pid)
{
    ...
    {
    // sigsuspend函数暂时用mask替换当前的阻塞集合，然后挂起该进程
    //因为mask为空，不阻塞所有进程，当接受任意信号，进行判断，
        sigsuspend(&mask);
    }
