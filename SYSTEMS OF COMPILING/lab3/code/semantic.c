...
void initTable() {
    ...
    symbolTable = NULL;
    // 添加read和write函数
    TableItem readFun = newAvlNode(newString("read"));
    readFun->type = newType(FUNCTION, 2, 0, newType(BASIC, 1, INT_TYPE));

    TableItem writeFun = newAvlNode(newString("write"));
    writeFun->type = newType(FUNCTION, 2, 1, newType(BASIC, 1, INT_TYPE));
    TableItem argc1 = newAvlNode("argc1");
    argc1->type = newType(BASIC, 1, INT_TYPE);
    writeFun->next = argc1;

    symbolTable = insert(symbolTable, readFun);
    symbolTable = insert(symbolTable, argc1);
    symbolTable = insert(symbolTable, writeFun);
    unNamedStructNum = 0;
}
...
void ExtDecList(pNode node, pType specifier) {
    ...
    while (temp) {
        ...
        if (findConflict(symbolTable, item->name)) {
            ...
            pError(RE_DEF_VAR, temp->lineNo, msg);
            delAvlNode(item);
        } else {
...
pType StructSpecifier(pNode node) {
    ...
    if (strcmp(t->name, "Tag") != 0) {
        TableItem structItem = newAvlNode("");
...
void FunDec(pNode node, pType returnType) {
    ...
    TableItem p = newAvlNode(node->child->val);
    ...
    if (findConflict(symbolTable, p->name)) {
        ...
        pError(RE_DEF_FUNC, node->lineNo, msg);
        delAvlNode(p);
        p = NULL;
    } else {
...
TableItem ParamDec(pNode node) {
    ...
    if (findConflict(symbolTable, p->name)) {
        ...
        pError(RE_DEF_VAR, node->lineNo, msg);
        delAvlNode(p);
        return NULL;
    } else {
        p->isArg = TRUE;
    ...
    if (node->child->next == NULL) {
        if (structInfo != NULL) {
            ...
            if (findConflict(symbolTable, decItem->name)) {
                ...
                pError(RE_DEF_FIELD, node->lineNo, msg);
                delAvlNode(decItem);
                return;
            } else {
        ...
        if (structInfo != NULL) {
            // 结构体内不能赋值，报错
            pError(RE_DEF_FIELD, node->lineNo,
    ...
    if (!strcmp(t->name, "Exp")) {
            ...
            if (!strcmp(t->next->name, "LB")) {
                ...
                if (!p1 || p1->kind != STRUCTURE ||
                    ...
                    if (structField == NULL) {
                        ...
                        pError(NON_EXIST_FIELD, t->lineNo, msg);
                    } else {
        ...
        if (funcInfo == NULL) {
            ...
            if (funcInfo->type->u.function.argc != 0) {
                ...
                pError(FUNC_ARGC_MISMATCH, node->lineNo, msg);
            }
            return copyType(funcInfo->type->u.function.returnType);
        }
    }
